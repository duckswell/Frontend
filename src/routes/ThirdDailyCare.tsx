...
export default function ThirdDailyCare() {
  ...
  useEffect(() => {
    ...
    const fetchRoutineSteps =
      async () => {
        try {
          const steps =
            await routineApi.getRoutineSteps(
              routine.routineId
            );
  ...
  async function handleCompleteDailyCareRoutine() {
    ...
    try {
      ...
      /*
       * 3. DAILY course 종료하지 않고
       * 오늘 루틴 요약 페이지로 이동
       */
      navigate(
        "/care/today_routine_summary",
        {
          state: {
            routineId:
              routine.routineId,
